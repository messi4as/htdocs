...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt_compra->bind_param("sssdss", $nome_cartao, $data_compra, $quantidade_parcelas, $valor_total, $descricao, $anexo_json); // Alterado para "sssdss"

    if ($stmt_compra->execute()) {
        $id_compra = $conn->insert_id;

        // Calcular e inserir os parcelamentos
        ...
        for ($i = 1; $i <= $quantidade_parcelas; $i++) {
            ...
            $valor_resp1 = round($valor_parcela * $percentual_responsavel1, 2);
            $valor_resp2 = round($valor_parcela * $percentual_responsavel2, 2);
            ...
            $stmt_parcela->bind_param("isddd", $id_compra, $data_vencimento_formatada, $referencia_parcela, $valor_resp1, $valor_resp2); // Alterado para "isddd"
