...
$responsaveis = $conn->query("SELECT DISTINCT responsavel FROM utilizacoes WHERE status_atualizado = 'utilizado'");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($cartoes as $numero) {
        $numero = trim($numero);
        $sql = "SELECT id FROM cartoes WHERE numero = '$numero' AND status = 'utilizado'";
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "SELECT nome, funcao FROM utilizacoes WHERE cartao_id = $cartao_id AND status_atualizado = 'utilizado' ORDER BY data_utilizacao DESC LIMIT 1";
            $result = $conn->query($sql);
            $row = $result->fetch_assoc();
            $nome = $row['nome'];
            $funcao = $row['funcao'];

            $status_atualizado = 'fechamento';
            ...
            $conn->query($sql);
            $mensagem_sucesso .= "Cartão $numero fechado com sucesso.<br>";
        } else {
            $mensagem_erro .= "Cartão $numero não está disponível para fechamento.<br>";
        }
    }
}
?>
...
<head>
    ...
    <title>Fechamento de Cartões</title>
                    ...
                    <div class="table-container">
                        ...
                        <div class="card-body">
                            ...
                            <form method="POST" action="">
                                ...
                                <div class="form-container">
                                    <div class="form-group">
                                        <label for="data_utilizacao" class="form-label">Data de Fechamento:</label>
                                ...
                                <button type="submit" class="btn btn-success"><span class="bi-save"></span>&nbsp;Registrar Fechamento</button>
                            </form>
