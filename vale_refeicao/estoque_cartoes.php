...
$responsaveis = $conn->query("SELECT DISTINCT responsavel FROM utilizacoes WHERE status_atualizado IN ('fechamento', 'devolvido', 'extraviado')");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($cartoes as $numero) {
        $numero = trim($numero);
        $sql = "SELECT id FROM cartoes WHERE numero = '$numero' AND status IN ('fechamento', 'devolvido', 'extraviado')";
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "SELECT nome, funcao FROM utilizacoes WHERE cartao_id = $cartao_id AND status_atualizado IN ('fechamento', 'devolvido', 'extraviado') ORDER BY data_utilizacao DESC LIMIT 1";
            $result = $conn->query($sql);
            $row = $result->fetch_assoc();
            $nome = $row['nome'];
            $funcao = $row['funcao'];

            $status_atualizado = 'estoque';
            ...
            $conn->query($sql);
            $mensagem_sucesso .= "Cartão $numero estocado com sucesso.<br>";
        } else {
            $mensagem_erro .= "Cartão $numero não está disponível para estocagem.<br>";
        }
    }
}
?>
...
<head>
    ...
    <title>Estoque de Cartões</title>
                    ...
                    <div class="table-container">
                        ...
                        <div class="card-body">
                            ...
                            <form method="POST" action="">
                                ...
                                <div class="form-container">
                                    <div class="form-group">
                                        <label for="data_utilizacao" class="form-label">Data de Estoque:</label>
                                ...
                                <button type="submit" class="btn btn-success"><span class="bi-save"></span>&nbsp;Registrar Estoque</button>
                            </form>
