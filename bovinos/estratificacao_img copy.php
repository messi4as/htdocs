...
// 1. Recuperar os agrupamentos para o filtro
...
$agrupamento_result = mysqli_query($conn, $agrupamento_query);
?>
<!doctype html>
<html lang="pt-br">
...
<head>
    ...
    <title>RELATÓRIO - FAZENDA ROSADA</title>
...
<body>
    ...
    <div class="container mt-4">
        ...
        <div class="row">
                ...
                <div class="card">
                    <div class="table-container">
                        <div class="card-header">
                            <h4>LISTAGEM POR AGRUPAMENTO
                                  <div class="float-end">
                                        <a href="visualizacao_graficos.php" class="btn btn-warning shadow-sm">Ver Gráficos</a>
            ...
            <div class="card-body">
                ...
                <form method="GET" class="row g-2 mb-4 p-3 bg-light rounded">
                    ...
                    <div class="col-md-5">
                        <select name="agrupamento" class="form-select">
                            ...
                            <?php 
                            mysqli_data_seek($agrupamento_result, 0);
                            while ($row = mysqli_fetch_assoc($agrupamento_result)): 
                            ?>
                                <option value="<?= $row['agrupamento'] ?>" <?= (isset($_GET['agrupamento']) && $_GET['agrupamento'] == $row['agrupamento']) ? 'selected' : '' ?>>
                                    <?= $row['agrupamento'] ?>
                                </option>
                            <?php endwhile; ?>
                        </select>
                ...
                $agrupamento = mysqli_real_escape_string($conn, $_GET['agrupamento'] ?? '');
                ...
                if ($agrupamento != '') $sql .= " AND agrupamento = '$agrupamento'";
                ...
                <table id="tabelaBovinos" class="table table-bordered align-middle w-100">
                    ...
                    <tbody>
                        <?php while ($bovino = mysqli_fetch_assoc($result)): 
                            // Verifica se existe o caminho no banco, se não, usa a padrão no diretório correto
  ...
  <script>
    ...
    // Função de conversão equilibrada (Nitidez vs Velocidade)
    function getBase64Image(img) {
        try {
            var canvas = document.createElement("canvas");
            // 250x250 é o "ponto doce" para fotos grandes sem travar o navegador
            canvas.width = 250; 
            canvas.height = 250;
            ...
            buttons: [
                {
                    ...
                    customize: function(doc) {
                        // 1. Capturar o filtro de Agrupamento com segurança
                        var valorFiltro = $('select[name="agrupamento"]').val();
                        var textoFiltro = (valorFiltro && valorFiltro !== "") ? 
                                          $('select[name="agrupamento"] option:selected').text() : 
                                          "Todos os Grupos";

                        // 2. Ajuste de Margens
                        doc.pageMargins = [30, 30, 30, 30]; 

                        // 3. ADICIONAR FILTRO (Forma segura que não trava o PDF)
                        // Adiciona o subtítulo no início do conteúdo
                        doc.content.unshift({
                            text: 'Agrupamento: ' + textoFiltro,
                            fontSize: 12,
                            italics: true,
                            alignment: 'center',
                            margin: [0, 0, 0, 20]
                        ...
                        // 4. Configurar a Tabela (Agora ela é o doc.content[2] porque adicionamos o filtro acima)
